...
--
myWorkspaces    = ["1:web","2:text", "3:ide","4:vm","5:media","6","7","8:msg","9:im"]
...
myKeys conf@(XConfig {XMonad.modMask = modMask}) = M.fromList $
   ...
   --  Reset the layouts on the current workspace to default
    ...
    , ((modMask .|. shiftMask, xK_Return), withFocused $ windows . W.sink)
...
myManageHook = composeAll
    ...
    , resource =? "sublime"  --> doShift "2:text"
    , resource =? "gtg"  --> doShift "9:im"
    , className =? "thunderbird" --> doShift "8:msg"
    , resource =? "eclipse"          --> doShift "3:ide"
    , className =? "VirtualBox"     --> doShift "4:vm"
    , className =? "banshee-1"      --> doShift "5:media"
    , className =? "Ktorrent"       --> doShift "5:media"
    , className =? "Pidgin"         --> doShift "9:im"
    , className =? "Xchat"          --> doShift "9:im"
    , className =? "Empathy"          --> doShift "9:im"
...
--
main = do
	xmproc <- spawnPipe "/usr/bin/xmobar ~/.xmonad/xmoba1r"
	xmonad $ defaults {
